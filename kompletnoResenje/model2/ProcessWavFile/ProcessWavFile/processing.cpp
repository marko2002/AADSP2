...
void processing(inverter_data_t* inverter)
{
	...
	{
		//Left side
		*p =(*p) * FRACT_NUM(GAIN_MINUS_6D); 
		*buffL++ = *p;
		//Right side
		*(p+BLOCK_SIZE) = *(p+BLOCK_SIZE) * FRACT_NUM(GAIN_MINUS_6D);
		*buffR++ = *(p+BLOCK_SIZE);
	}
	...
	for(p = sampleBuffer[0]; p < sampleBuffer[0] + BLOCK_SIZE; p++)
	{
		*(p+32) = (*p + *(p+16))*FRACT_NUM(GAIN_MINUS_3D);
	}
	...
	for(p = sampleBuffer[0]; p < sampleBuffer[0] + BLOCK_SIZE; p++){
		...
		*p = *(p+32) * FRACT_NUM(GAIN_MINUS_6D);
		*(p+16) = *(p+32) * FRACT_NUM(GAIN_MINUS_6D);
			
		*(p+48) = (*buffL*FRACT_NUM(GAIN_MINUS_2D)) + (*p);
		*(p+64) = (*buffR*FRACT_NUM(GAIN_MINUS_2D)) + (*(p+16));
